...
void ZGLInput::Update() {
    ...
    if (mousePress_[GLFW_MOUSE_BUTTON_LEFT]) {
        if (yaw != lastYaw_ || pitch != lastPitch_) {
            std::shared_ptr<ZObjectDragEvent> dragEvent(new ZObjectDragEvent(yaw - lastYaw_, lastPitch_ - pitch, 0.f));
    ...
    if (yaw != lastYaw_ || pitch != lastPitch_) {
        std::shared_ptr<ZObjectLookEvent> lookEvent(new ZObjectLookEvent(yaw - lastYaw_, lastPitch_ - pitch));
