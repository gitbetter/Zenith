...
void main() {
  ...
  if (light.isEnabled) {
      ...
      diffuse += (1.0 - shadow) * light.color.rgb * material.diffuse * albd.rgb * diff * attenunation;
      specular += (1.0 - shadow) * light.color.rgb * material.specular * spec * attenunation;
  }

  vec3 color = (ambient + diffuse + specular) + material.emission;
